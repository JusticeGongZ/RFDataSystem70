package RealTimeDisplayWithAmpAndPhase.util;
 ...
 * @since 1.2
 */
public class FFTCalculate implements Serializable {
     ...
     * transformed data
     */
    public FFTCalculate(final DftNormalization normalization) {
    ...
    public double[] transform(final double[] f, final TransformType type) {
        ...
        return createComplexArray(dataRI);
    }

    public static double[] createComplexArray(final double[][] dataRI)
        ...
        for (int i = 0; i < n; i++) {
            c[i] = Math.sqrt(Math.pow(dataR[i], 2) + Math.pow(dataI[i], 2));
        }
        return c;
    }
