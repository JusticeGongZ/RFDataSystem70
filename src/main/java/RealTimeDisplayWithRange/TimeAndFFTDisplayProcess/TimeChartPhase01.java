...
//        numberaxis.setLowerBound(2);
//            numberaxis.setRange(-2,2);
...
//        chartPanel.setSize(100,100);
         ...
         * 随机生成的数据
         */
        public static void dynamicRun(double[] data, double avg) {

            int i = 0;
            int j = 0;

            while (true) {
                jfreechart.setTitle("通道1相位");
                jfreechart.getTitle().setFont(new Font("微软雅黑", 0, 16));//设置标题字体

                for (int index = 0; index < 1000; index++){
                    xyCPUseries.add(index, data[index] - avg);
                }

                while (true){
                    xyCPUseries.updateByIndex(i, data[j] - avg);
                    try {
